...
import { ArrowUpFromLine, Plus, Minus, Save, TruckIcon, Calendar as CalendarIcon, Clock } from 'lucide-react';
...
export default function LogTruckOut() {
  ...
  // Only show trucks that are IN or AVAILABLE (to log them OUT)
  useEffect(() => {
    if (trucks.length > 0) {
      const trucksToLogOut = trucks.filter(truck => 
        truck.currentStatus === 'IN' || truck.currentStatus === 'AVAILABLE'
      );
      const initialCounts = trucksToLogOut.map(truck => ({
        truckId: truck._id,
  ...
  const handleSave = async () => {
    if (totalCount === 0) {
      toast.error('No exits to save', {
        description: 'Please add at least one truck exit before saving',
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to save exits');
      }

      const result = await response.json();
      
      toast.success('Exits saved successfully!', {
        description: `Saved ${result.count} truck ${result.count === 1 ? 'exit' : 'exits'} for ${format(date, 'PPP')}`,
      });
      ...
      toast.error('Failed to save exits', {
    ...
    <div className="min-h-screen bg-white p-4 md:p-6 lg:p-8">
      <div className="w-full space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight flex items-center gap-2">
              <ArrowUpFromLine className="h-6 w-6 md:h-8 md:w-8 text-red-600" />
              Log Truck OUT
            </h1>
            <p className="text-sm text-muted-foreground mt-1">
              Count trucks exiting the quarry on {format(date, 'EEEE, MMMM d, yyyy')}
            </p>
          </div>
          <Popover>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <CardTitle className="text-lg">Total Exits Today</CardTitle>
                <CardDescription>Trucks logged exiting the quarry</CardDescription>
              </div>
              <div className="flex flex-col items-end gap-2">
                <div className="text-4xl font-bold text-red-600">{totalCount}</div>
                <div className="text-right">
                  <div className="text-sm font-semibold text-slate-700">{format(date, 'MMM dd, yyyy')}</div>
                  <div className="text-lg font-mono font-bold text-red-600">{format(currentTime, 'hh:mm:ss a')}</div>
                </div>
              </div>
            </div>
          </CardHeader>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-end flex-wrap gap-3">
          ...
          <Button onClick={handleSave} disabled={isSaving || totalCount === 0} className="gap-2 bg-red-600 hover:bg-red-700 w-full sm:w-auto">
            <Save className="h-4 w-4" />
            {isSaving ? 'Saving...' : 'Save Exit Log'}
          </Button>
        </div>
        ...
        <div className="md:hidden space-y-4">
          {truckCounts.map((tc) => (
            <Card key={tc.truckId} className="border-l-4 border-l-red-400">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <Badge variant={tc.count > 0 ? "default" : "outline"} className={tc.count > 0 ? "bg-red-500" : ""}>
                    {tc.count} exits
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-red-600">Exit Count</Label>
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className="h-12 w-12 p-0 border-red-500 text-red-600 hover:bg-red-50"
                    >
        ...
        <Card className="hidden md:block">
          <CardHeader>
            ...
            <CardDescription>Count how many times each truck exited today</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {truckCounts.map((tc) => (
                <div key={tc.truckId} className="flex items-center gap-4 p-4 border rounded-lg hover:bg-slate-50 transition-colors">
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <Label className="text-xs font-medium text-red-600">EXITS</Label>
                    <div className="flex items-center gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => handleIncrement(tc.truckId)}
                        className="h-9 w-9 p-0 border-red-500 text-red-600 hover:bg-red-50"
                      >
                  ...
                  <Badge variant={tc.count > 0 ? "default" : "secondary"} className={tc.count > 0 ? "bg-red-500" : ""}>
                    {tc.count} {tc.count === 1 ? 'exit' : 'exits'}
                  </Badge>
                </div>
              ))}
            </div>
