...
export default function ManualTruckOutLogsPage() {
  ...
  const [truckStatus, setTruckStatus] = useState<'empty' | 'half-loaded' | 'full'>('empty');
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/admin-truck-logs?logType=out`,
        {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-orange-900 to-orange-600 bg-clip-text text-transparent">
            Manual Truck Out Counter
          </h2>
          <p className="text-sm text-slate-500 mt-1">Click + button to count each truck leaving</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card className="border-l-4 border-l-red-500">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-slate-600">Total Trucks Out</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-red-600">{totalTrucks}</div>
      ...
      <Tabs defaultValue="quarries" className="space-y-4">
        ...
        <TabsContent value="quarries" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Click + to Count Trucks Out</CardTitle>
              <CardDescription>Each click records 1 truck leaving</CardDescription>
            </CardHeader>
            <CardContent>
          {isLoading ? (
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      ...
                      className="bg-orange-600 hover:bg-orange-700 text-white w-full h-10 text-sm font-semibold"
                    >
                      {isCountingThis ? (
                        <>
        ...
        <CardHeader>
          ...
          <CardDescription>Last 20 truck exits recorded</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={confirmDialog.open} onOpenChange={(open) => setConfirmDialog({ open, quarry: null })}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to count 1 truck leaving this quarry?
            </DialogDescription>
          </DialogHeader>
